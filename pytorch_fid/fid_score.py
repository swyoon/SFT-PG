...
class Crop(object):
    ...
    def __call__(self, img):
        return TF.functional.crop(img, self.x1, self.y1, self.x2 - self.x1, self.y2 - self.y1)
...
def compute_statistics_of_dataloader(ds, batch_size, dims, device):

    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    model = InceptionV3([block_idx]).to(device)
    model.eval()

    if ds == 'lsun_bedroom_train':
        from torchvision.datasets import LSUN
        # seed following https://github.com/ermongroup/ddim
        random_state = np.random.get_state()
        np.random.seed(2019)
        np.random.set_state(random_state)
        torch.manual_seed(1234)
        dataset = LSUN('/tmp2/LSUN', ['bedroom_train'], transform=TF.Compose([TF.Resize(256),
                                                                              TF.CenterCrop(256),
                                                                              TF.RandomHorizontalFlip(p=0.5),
                                                                              TF.ToTensor()])) 
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=1)
    elif ds == 'celeba64_train':
        from torchvision.datasets import CelebA
        # seed and preprocess following https://github.com/ermongroup/ddim
        random_state = np.random.get_state()
        np.random.seed(2019)
        np.random.set_state(random_state)
        torch.manual_seed(1234)
        ...
        y1 = cx - 64
        y2 = cx + 64
        dataset = CelebA('/tmp2/celeba64', split='train', transform=TF.Compose([Crop(x1, x2, y1, y2), 
